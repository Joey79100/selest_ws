<?php
	...
	*/
	...
	if (isset($_GET["id_conversation"])) {
		...
		/**
		 * Vérification que l'utlisateur fait bien partie de cette conversation
		 */

		// préparation de la requête
		$query = "SELECT COUNT(*) AS utilisateur_present
		...
		try{
			...
			$utilisateur_present = $stmt->fetchAll(PDO::FETCH_ASSOC)[0]['utilisateur_present'];

		} catch(PDOException $e) {

			stopWithError($e, "Erreur lors de la vérification de la présence de l'utilisteur dans la conversation");

		}


		
		// On ne récupère les messages que si l'utilisateur fait partie de la conversation
		if($utilisateur_present){
			...
			// pas de donnée
			$response["success"] = 0;
			$response["message"] = "Requête impossible - l'utilisateur ne fait pas partie de la conversation";
			$code = CODE_FORBIDDEN;
